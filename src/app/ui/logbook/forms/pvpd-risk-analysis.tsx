...
export default function PVPDRiskAnalysis({
    ...
    const offlineUseEffect = async () => {
        // getLogBookEntryByID(+logentryID, handleSetLogbook)
        const data = await logBookEntryModel.getById(+logentryID)
    ...
    const handleDgrFieldChange = (field: string) => async (check: Boolean) => {
        ...
        if (+currentEvent.dangerousGoodsChecklist.id > 0) {
            if (offline) {
                // updateDangerousGoodsChecklistNoRefresh
                await dangerousGoodsChecklistModel.save({
                    id: currentEvent.dangerousGoodsChecklist.id,
    ...
    const fields = [
        {
            ...
            checked: riskBuffer?.vesselSecuredToWharf
    ...
    useEffect(() => {
        setstrategyEditor(<div className="h-[18.3rem]"></div>)
        const strategyEditor = (
            <Editor
                id="strategy"
                placeholder="Mitigation strategy identified"
                className="w-full"
                content={content}
        ...
        setTimeout(() => {
            setstrategyEditor(strategyEditor)
        }, 10)
    }, [recommendedstrategy, currentRisk, openRiskDialog])
