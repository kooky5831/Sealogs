...
export default function Cleaning({
    ...
    const [exterior, setExterior] = useState<Boolean>(
        vesselDailyCheck?.exterior === 'Ok',
    )
    const [interior, setInterior] = useState<Boolean>(
        vesselDailyCheck?.interior === 'Ok',
    )

    const handleExterior = async (check: Boolean) => {
        setExterior(check)
        if (+vesselDailyCheck?.id > 0) {
            const variables = {
                id: vesselDailyCheck.id,
                exterior: check ? 'Ok' : 'Not_Ok',
            }
            if (offline) {
    ...
    const handleInterior = async (check: Boolean) => {
        setInterior(check)
        if (+vesselDailyCheck?.id > 0) {
            const variables = {
                id: vesselDailyCheck.id,
                interior: check ? 'Ok' : 'Not_Ok',
            }
            if (offline) {
                const data = await dailyCheckModel.save(variables)
                setVesselDailyCheck([data])
    ...
    const handleSave = async () => {
        ...
        toast.loading('Saving Cleaning Checks...')
        ...
        <>
            <div className="bg-white p-6 border border-slblue-100 rounded-lg dark:bg-sldarkblue-800">
                <div className="flex w-full justify-between items-center">
                    <h3 className="dark:text-white">Cleaning Checks</h3>
                </div>
