...
export default function Documentation({
    ...
    const [charts, setCharts] = useState<Boolean>(
        vesselDailyCheck?.charts === 'Ok',
    )
    const [documentCrewBriefings, setDocumentCrewBriefings] = useState<Boolean>(
        vesselDailyCheck?.documentCrewBriefings === 'Ok',
    )
    const [recordComments, setRecordComments] = useState<Boolean>(
        vesselDailyCheck?.recordComments === 'Ok',
    )
    ...
    const handleCharts = async (check: Boolean) => {
        setCharts(check)
        if (+vesselDailyCheck?.id > 0) {
            const variables = {
                id: vesselDailyCheck.id,
                charts: check ? 'Ok' : 'Not_Ok',
            }
            if (offline) {
    ...
    const handleDocumentCrewBriefings = async (check: Boolean) => {
        setDocumentCrewBriefings(check)
        if (+vesselDailyCheck?.id > 0) {
            const variables = {
                id: vesselDailyCheck.id,
                documentCrewBriefings: check ? 'Ok' : 'Not_Ok',
            }
            if (offline) {
                const data = await dailyCheckModel.save(variables)
                setVesselDailyCheck([data])
    ...
    const handleRecordComments = async (check: Boolean) => {
        setRecordComments(check)
        if (+vesselDailyCheck?.id > 0) {
            const variables = {
                id: vesselDailyCheck.id,
                recordComments: check ? 'Ok' : 'Not_Ok',
            }
            if (offline) {
                const data = await dailyCheckModel.save(variables)
                setVesselDailyCheck([data])
    ...
    const fields = [
        {
            name: 'Charts',
            label: 'Update Charts and Almanac, Cargo Manifest, Compliance with Stability Book',
            value: 'charts',
            checked: vesselDailyCheck?.charts,
            sortOrder: getSortOrder('Charts', logBookConfig),
            handleChange: handleCharts,
        },
        {
            name: 'DocumentCrewBriefings',
            label: 'Document Crew Briefings, Meeting Notes, Vessel Certificates',
            value: 'documentCrewBriefings',
            checked: vesselDailyCheck?.documentCrewBriefings,
            sortOrder: getSortOrder('DocumentCrewBriefings', logBookConfig),
            handleChange: handleDocumentCrewBriefings,
        },
        {
            name: 'RecordComments',
            label: 'Record Comments and Daily Logbook Entries',
            value: 'recordComments',
            checked: vesselDailyCheck?.recordComments,
            sortOrder: getSortOrder('RecordComments', logBookConfig),
            handleChange: handleRecordComments,
        },
    ]
    ...
    const handleSave = async () => {
        ...
        toast.loading('Saving Documentation...')
        ...
        <>
            <div className="bg-white p-6 border border-slblue-100 rounded-lg dark:bg-sldarkblue-800">
                <div className="flex w-full justify-between items-center">
                    <h3 className="dark:text-white">Documentation</h3>
                </div>
                    ...
                    <div className="flex flex-col grid-cols-1 md:col-span-2 lg:col-span-3">
                        <div className="flex items-center justify-between">
                            <textarea
                                ...
                                /* onBlur={(e) =>
                                    getComment('Documentation', 'Section')?.id >
                                    0
                                        ? updateSectionMemberComment({
                                              variables: {
                                                  input: {
                                ...
                                defaultValue={
                                    getComment('Documentation', 'Section')
                                        ?.comment
                                }></textarea>
                        </div>
                    </div>
