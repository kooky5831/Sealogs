...
import { getInventoryCategoryByID } from '@/app/lib/actions'
import { classes } from '@/app/components/GlobalClasses'

export default function EditInventoryCategory({
    categoryID,
}: {
    categoryID: number
}) {
    ...
    getInventoryCategoryByID(categoryID, setCategory)
    ...
    const handleUpdate = async () => {
        ...
        if (categoryName !== '') {
            return await mutationupdateInventoryCategory({
                variables,
            })
        }
    }
    ...
    const [
        mutationupdateInventoryCategory,
        { loading: mutationupdateInventoryCategoryLoading },
    ] = useMutation(UPDATE_INVENTORY_CATEGORY, {
        onCompleted: (response: any) => {
            const data = response.updateInventoryCategory
            if (data.id > 0) {
                router.back()
            } else {
                console.error('mutationupdateInventoryCategory error', response)
            }
        },
        onError: (error: any) => {
            console.error('mutationupdateInventoryCategory error', error)
        },
    })

    const handleDeleteCategory = async (categoryId: number) => {
        await mutationDeleteInventoryCategory({
            variables: {
                id: categoryId,
            },
        })
    ...
    const [
        mutationDeleteInventoryCategory,
        { loading: mutationDeleteInventoryCategoryLoading },
    ] = useMutation(DELETE_INVENTORY_CATEGORY, {
        onCompleted: (response: any) => {
            const { isSuccess, data } = response.deleteInventoryCategory
            if (isSuccess) {
                // loadInventoryCategory()
            } else {
                console.error('mutationDeleteInventoryCategory error', response)
            }
        },
        onError: (error: any) => {
            console.error('mutationDeleteInventoryCategory error', error)
        },
    })
        ...
        <div className="w-full p-0 dark:text-white">
            ...
            <FooterWrapper>
                <SeaLogsButton
                    text="Cancel"
                    type="text"
                    // link="/settings/inventory/category"
                    action={() => {
                        router.back()
                    }}
                />
