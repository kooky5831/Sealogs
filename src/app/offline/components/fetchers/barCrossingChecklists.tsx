import { DownloadBarCrossingChecklists } from '@/app/lib/graphQL/query/offline/downloadBarCrossingChecklists'
...
const FetchBarCrossingChecklists : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'bar crossing checklists'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new BarCrossingChecklistModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadBarCrossingChecklists, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readBarCrossingChecklists.nodes
            const hasNextPage =
                response.readBarCrossingChecklists.pageInfo.hasNextPage
            const totalCount =
                response.readBarCrossingChecklists.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('BarCrossingChecklists', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('BarCrossingChecklists');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('BarCrossingChecklists','fetch');
            setStorageItem('BarCrossingChecklists', 'success', '100','fetch');
        } else { 
            setStorageItem('BarCrossingChecklists', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchBarCrossingChecklists
