import { DownloadEventType_PersonRescues } from '@/app/lib/graphQL/query/offline/downloadEventType_PersonRescues'
...
const FetchEventType_PersonRescues : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'person rescue event types'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new EventType_PersonRescueModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadEventType_PersonRescues, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readEventType_PersonRescues.nodes
            const hasNextPage =
                response.readEventType_PersonRescues.pageInfo.hasNextPage
            const totalCount =
                response.readEventType_PersonRescues.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('EventType_PersonRescues', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('EventType_PersonRescues');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('EventType_PersonRescues','fetch');
            setStorageItem('EventType_PersonRescues', 'success', '100','fetch');
        } else { 
            setStorageItem('EventType_PersonRescues', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchEventType_PersonRescues
