import { DownloadInventories } from '@/app/lib/graphQL/query/offline/downloadInventories'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import InventoryModel from '@/app/offline/models/inventory'
import dayjs from 'dayjs'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
// REMEMBER: The indexedDB is the source of truth

const FetchInventories : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadInventories, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readInventories.nodes
            const hasNextPage = response.readInventories.pageInfo.hasNextPage
            const totalCount = response.readInventories.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('Inventories', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('Inventories');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('Inventories','fetch');
            setStorageItem('Inventories', 'success', '100','fetch');
        } else { 
            setStorageItem('Inventories', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
