import { DownloadCGEventMissions } from '@/app/lib/graphQL/query/offline/downloadCGEventMissions'
...
// cgEventMission
const FetchCGEventMissions : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'event missions'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new CGEventMissionModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadCGEventMissions, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readCGEventMissions.nodes
            const hasNextPage =
                response.readCGEventMissions.pageInfo.hasNextPage
            const totalCount = response.readCGEventMissions.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('CGEventMissions', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('CGEventMissions');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('CGEventMissions','fetch');
            setStorageItem('CGEventMissions', 'success', '100','fetch');
        } else { 
            setStorageItem('CGEventMissions', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchCGEventMissions
