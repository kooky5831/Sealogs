import { DownloadRefuellingBunkerings } from '@/app/lib/graphQL/query/offline/downloadRefuellingBunkerings'
...
const FetchRefuellingBunkerings : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'refuelling bunkerings'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new RefuellingBunkeringModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadRefuellingBunkerings, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readRefuellingBunkerings.nodes
            const hasNextPage =
                response.readRefuellingBunkerings.pageInfo.hasNextPage
            const totalCount =
                response.readRefuellingBunkerings.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('RefuellingBunkerings', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('RefuellingBunkerings');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('RefuellingBunkerings','fetch');
            setStorageItem('RefuellingBunkerings', 'success', '100','fetch');
        } else { 
            setStorageItem('RefuellingBunkerings', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchRefuellingBunkerings
