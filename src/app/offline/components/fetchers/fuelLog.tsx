import { DownloadFuelLog } from '@/app/lib/graphQL/query/offline/downloadFuelLog'
...
const FetchFuelLog : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadFuelLog, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readFuelLogs.nodes
            const hasNextPage = response.readFuelLogs.pageInfo.hasNextPage
            const totalCount = response.readFuelLogs.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('FuelLog', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('FuelLog');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('FuelLog','fetch');
            setStorageItem('FuelLog', 'success', '100','fetch');
        } else {
            setStorageItem('FuelLog', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
