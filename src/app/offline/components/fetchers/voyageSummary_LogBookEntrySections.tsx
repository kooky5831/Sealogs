import { DownloadVoyageSummary_LogBookEntrySections } from '@/app/lib/graphQL/query/offline/downloadVoyageSummary_LogBookEntrySections'
...
const FetchVoyageSummary_LogBookEntrySections : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'voyage summary sections'
    ...
    const [queryData] = useLazyQuery(
        DownloadVoyageSummary_LogBookEntrySections,
        {
            ...
            onCompleted: (response: any) => {
                ...
                const hasNextPage =
                    response.readVoyageSummary_LogBookEntrySections.pageInfo
                        .hasNextPage
                const totalCount =
                    response.readVoyageSummary_LogBookEntrySections.pageInfo
                        .totalCount
            ...
            onError: (error: any) => {
                ...
                if(typeof window !== 'undefined') {
                    setStorageItem('VoyageSummary_LogBookEntrySections', 'error', `${progressBarPercentValue}`,'fetch')
                    setFetchError('VoyageSummary_LogBookEntrySections');
                }
            },
        },
    )
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('VoyageSummary_LogBookEntrySections','fetch');
            setStorageItem('VoyageSummary_LogBookEntrySections', 'success', '100','fetch');
        } else {
            setStorageItem('VoyageSummary_LogBookEntrySections', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchVoyageSummary_LogBookEntrySections
