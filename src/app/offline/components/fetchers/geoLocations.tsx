import { DownloadGeoLocations } from '@/app/lib/graphQL/query/offline/downloadGeoLocations'
...
const FetchGeoLocations : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'geolocations'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new GeoLocationModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadGeoLocations, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readGeoLocations.nodes
            const hasNextPage = response.readGeoLocations.pageInfo.hasNextPage
            const totalCount = response.readGeoLocations.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('GeoLocations', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('GeoLocations');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('GeoLocations','fetch');
            setStorageItem('GeoLocations', 'success', '100','fetch');
        } else {
            setStorageItem('GeoLocations', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
