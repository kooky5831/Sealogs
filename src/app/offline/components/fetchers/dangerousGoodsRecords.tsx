import { DownloadDangerousGoodsRecords } from '@/app/lib/graphQL/query/offline/downloadDangerousGoodsRecords'
...
const FetchDangerousGoodsRecords : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'DangerousGoodsRecords'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new DangerousGoodsRecordModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadDangerousGoodsRecords, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readDangerousGoodsRecords.nodes
            const hasNextPage =
                response.readDangerousGoodsRecords.pageInfo.hasNextPage
            const totalCount =
                response.readDangerousGoodsRecords.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('DangerousGoodsRecords', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('DangerousGoodsRecords');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('DangerousGoodsRecords','fetch');
            setStorageItem('DangerousGoodsRecords', 'success', '100','fetch');
        } else {
            setStorageItem('DangerousGoodsRecords', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchDangerousGoodsRecords
