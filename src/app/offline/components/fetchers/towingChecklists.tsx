import { DownloadTowingChecklists } from '@/app/lib/graphQL/query/offline/downloadTowingChecklists'
...
const FetchTowingChecklists : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'towing checklists'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new TowingChecklistModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadTowingChecklists, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readTowingChecklists.nodes
            const hasNextPage =
                response.readTowingChecklists.pageInfo.hasNextPage
            const totalCount = response.readTowingChecklists.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('TowingChecklists', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('TowingChecklists'); 
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('TowingChecklists','fetch');
            setStorageItem('TowingChecklists', 'success', '100','fetch');
        } else { 
            setStorageItem('TowingChecklists', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchTowingChecklists
