import { DownloadConsequences } from '@/app/lib/graphQL/query/offline/downloadConsequences'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import ConsequenceModel from '@/app/offline/models/consequence'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
import dayjs from 'dayjs'
// REMEMBER: The indexedDB is the source of truth

const FetchConsequences : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'consequences'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new ConsequenceModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadConsequences, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readConsequences.nodes
            const hasNextPage = response.readConsequences.pageInfo.hasNextPage
            const totalCount = response.readConsequences.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('Consequences', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('Consequences');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('Consequences','fetch');
            setStorageItem('Consequences', 'success', '100','fetch');
        } else { 
            setStorageItem('Consequences', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchConsequences
