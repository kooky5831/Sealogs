import { DownloadWeatherForecasts } from '@/app/lib/graphQL/query/offline/downloadWeatherForecasts'
...
// weatherForecast
const FetchWeatherForecasts : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadWeatherForecasts, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readWeatherForecasts.nodes
            const hasNextPage =
                response.readWeatherForecasts.pageInfo.hasNextPage
            const totalCount = response.readWeatherForecasts.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('WeatherForecasts', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('WeatherForecasts');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('WeatherForecasts','fetch');
            setStorageItem('WeatherForecasts', 'success', '100','fetch');
        } else {
            setStorageItem('WeatherForecasts', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
