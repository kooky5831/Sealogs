import { DownloadWeatherTides } from '@/app/lib/graphQL/query/offline/downloadWeatherTides'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import WeatherTideModel from '@/app/offline/models/weatherTide'
import dayjs from 'dayjs'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
// REMEMBER: The indexedDB is the source of truth
const FetchWeatherTides : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadWeatherTides, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readWeatherTides.nodes
            const hasNextPage = response.readWeatherTides.pageInfo.hasNextPage
            const totalCount = response.readWeatherTides.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('WeatherTides', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('WeatherTides');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('WeatherTides','fetch');
            setStorageItem('WeatherTides', 'success', '100','fetch');
        } else {
            setStorageItem('WeatherTides', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchWeatherTides
