import { DownloadDangerousGoods } from '@/app/lib/graphQL/query/offline/downloadDangerousGoods'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import DangerousGoodModel from '@/app/offline/models/dangerousGood'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
import dayjs from 'dayjs'
// REMEMBER: The indexedDB is the source of truth

const FetchDangerousGoods : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'dangerous goods'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new DangerousGoodModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadDangerousGoods, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readDangerousGoods.nodes
            const hasNextPage = response.readDangerousGoods.pageInfo.hasNextPage
            const totalCount = response.readDangerousGoods.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('DangerousGoods', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('DangerousGoods');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('DangerousGoods','fetch');
            setStorageItem('DangerousGoods', 'success', '100','fetch');
        } else { 
            setStorageItem('DangerousGoods', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchDangerousGoods
