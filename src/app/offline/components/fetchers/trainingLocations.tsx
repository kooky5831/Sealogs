import { DownloadTrainingLocations } from '@/app/lib/graphQL/query/offline/downloadTrainingLocations'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import TrainingLocationModel from '@/app/offline/models/trainingLocation'
...
const FetchTrainingLocations: React.FC<{ flag: string }> = React.memo(
    ({ flag }) => {
        const limit = 100
        const [progressBarPercentValue, setProgressBarPercentValue] =
            useState(0)
        const title = 'training locations'
        const [progressBarLabel, setProgressBarLabel] = useState(
            `Downloading ${title}...`,
        )
        const model = new TrainingLocationModel()
        const [page, setPage] = useState(0)
        const [queryData] = useLazyQuery(DownloadTrainingLocations, {
            fetchPolicy: 'network-only',
            onCompleted: (response: any) => {
                const data = response.readTrainingLocations.nodes
                const hasNextPage =
                    response.readTrainingLocations.pageInfo.hasNextPage
                const totalCount =
                    response.readTrainingLocations.pageInfo.totalCount
                if (data) {
            ...
            onError: (error: any) => {
                ...
                if (typeof window !== 'undefined') {
                    ...
                    setFetchError('TrainingLocations')
                }
            },
        })
        ...
        useEffect(() => {
            if (progressBarPercentValue == 100) {
                addSuccessResult('TrainingLocations','fetch')
                setStorageItem('TrainingLocations', 'success', '100','fetch')
            } else {
                setStorageItem(
                    'TrainingLocations',
                    'fetching',
...
export default FetchTrainingLocations
