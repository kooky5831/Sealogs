import { DownloadAssetReporting_LogBookEntrySections } from '@/app/lib/graphQL/query/offline/downloadAssetReporting_LogBookEntrySections'
...
const FetchAssetReporting_LogBookEntrySections : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'asset report sections'
    ...
    const [queryData] = useLazyQuery(
        DownloadAssetReporting_LogBookEntrySections,
        {
            ...
            onCompleted: (response: any) => {
                ...
                const hasNextPage =
                    response.readAssetReporting_LogBookEntrySections.pageInfo
                        .hasNextPage
                const totalCount =
                    response.readAssetReporting_LogBookEntrySections.pageInfo
                        .totalCount
            ...
            onError: (error: any) => {
                ...
                if(typeof window !== 'undefined') {
                    setStorageItem('AssetReporting_LogBookEntrySections', 'error', `${progressBarPercentValue}`,'fetch')
                    setFetchError('AssetReporting_LogBookEntrySections');
                }
            },
        },
    )
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('AssetReporting_LogBookEntrySections','fetch');
            setStorageItem('AssetReporting_LogBookEntrySections', 'success', '100','fetch');
        } else {
            setStorageItem('AssetReporting_LogBookEntrySections', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchAssetReporting_LogBookEntrySections
