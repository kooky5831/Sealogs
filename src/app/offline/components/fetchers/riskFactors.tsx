import { DownloadRiskFactors } from '@/app/lib/graphQL/query/offline/downloadRiskFactors'
...
const FetchRiskFactors : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadRiskFactors, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readRiskFactors.nodes
            const hasNextPage = response.readRiskFactors.pageInfo.hasNextPage
            const totalCount = response.readRiskFactors.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('RiskFactors', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('RiskFactors');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('RiskFactors','fetch');
            setStorageItem('RiskFactors', 'success', '100','fetch');
        } else { 
            setStorageItem('RiskFactors', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchRiskFactors
