import { DownloadEventTypes } from '@/app/lib/graphQL/query/offline/downloadEventTypes'
...
const FetchEventTypes: React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    ...
    const [queryData] = useLazyQuery(DownloadEventTypes, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readEventTypes.nodes
            const hasNextPage = response.readEventTypes.pageInfo.hasNextPage
            const totalCount = response.readEventTypes.pageInfo.totalCount
            if (data) {
    ...
    useEffect(() => {
        if (progressBarPercentValue == 100) {
            addSuccessResult('EventTypes','fetch')
            setStorageItem('EventTypes', 'success', '100','fetch')
        } else {
            setStorageItem(
                'EventTypes',
                'fetching',
...
export default FetchEventTypes
