import { DownloadSupernumerary_LogBookEntrySections } from '@/app/lib/graphQL/query/offline/downloadSupernumerary_LogBookEntrySections'
...
const FetchSupernumerary_LogBookEntrySections : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'supernumerary sections'
    ...
    const [queryData] = useLazyQuery(
        DownloadSupernumerary_LogBookEntrySections,
        {
            ...
            onCompleted: (response: any) => {
                ...
                const hasNextPage =
                    response.readSupernumerary_LogBookEntrySections.pageInfo
                        .hasNextPage
                const totalCount =
                    response.readSupernumerary_LogBookEntrySections.pageInfo
                        .totalCount
            ...
            onError: (error: any) => {
                ...
                if(typeof window !== 'undefined') {
                    setStorageItem(
                        'Supernumerary_LogBookEntrySections', 
                        'error', 
                        `${progressBarPercentValue}`,
                        'fetch'
                    )
                    setFetchError('Supernumerary_LogBookEntrySections');
                }
            },
        },
    )
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('Supernumerary_LogBookEntrySections','fetch');
            setStorageItem('Supernumerary_LogBookEntrySections', 'success', '100','fetch');
        } else {
            setStorageItem(
                'Supernumerary_LogBookEntrySections', 
                'fetching', 
...
export default FetchSupernumerary_LogBookEntrySections
