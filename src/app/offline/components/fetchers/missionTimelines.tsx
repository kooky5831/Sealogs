import { DownloadMissionTimelines } from '@/app/lib/graphQL/query/offline/downloadMissionTimelines'
...
const FetchMissionTimelines : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'mission timelines'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new MissionTimelineModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadMissionTimelines, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readMissionTimelines.nodes
            const hasNextPage =
                response.readMissionTimelines.pageInfo.hasNextPage
            const totalCount = response.readMissionTimelines.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('MissionTimelines', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('MissionTimelines');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('MissionTimelines','fetch');
            setStorageItem('MissionTimelines', 'success', '100','fetch');
        } else { 
            setStorageItem('MissionTimelines', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchMissionTimelines
