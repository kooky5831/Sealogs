import { DownloadComponentMaintenanceChecks } from '@/app/lib/graphQL/query/offline/downloadComponentMaintenanceChecks'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import ComponentMaintenanceChecksModel from '@/app/offline/models/componentMaintenanceCheck'
import dayjs from 'dayjs'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
// REMEMBER: The indexedDB is the source of truth
const FetchComponentMaintenanceChecks : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'component maintenance checks'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new ComponentMaintenanceChecksModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadComponentMaintenanceChecks, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readComponentMaintenanceChecks.nodes
            const hasNextPage =
                response.readComponentMaintenanceChecks.pageInfo.hasNextPage
            const totalCount =
                response.readComponentMaintenanceChecks.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('ComponentMaintenanceChecks', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('ComponentMaintenanceChecks');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('ComponentMaintenanceChecks','fetch');
            setStorageItem('ComponentMaintenanceChecks', 'success', '100','fetch');
        } else {
            setStorageItem('ComponentMaintenanceChecks', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchComponentMaintenanceChecks
