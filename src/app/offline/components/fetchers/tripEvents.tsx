import { DownloadTripEvents } from '@/app/lib/graphQL/query/offline/downloadTripEvents'
...
// tripEvent
const FetchTripEvents : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'trip events'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new TripEventModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadTripEvents, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readTripEvents.nodes
            const hasNextPage = response.readTripEvents.pageInfo.hasNextPage
            const totalCount = response.readTripEvents.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('TripEvents', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('TripEvents');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('TripEvents','fetch');
            setStorageItem('TripEvents', 'success', '100','fetch');
        } else { 
            setStorageItem('TripEvents', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchTripEvents
