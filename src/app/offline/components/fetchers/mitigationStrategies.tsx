import { DownloadMitigationStrategies } from '@/app/lib/graphQL/query/offline/downloadMitigationStrategies'
...
const FetchMitigationStrategies : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'mitigation strategies'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new MitigationStrategyModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadMitigationStrategies, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readMitigationStrategies.nodes
            const hasNextPage =
                response.readMitigationStrategies.pageInfo.hasNextPage
            const totalCount =
                response.readMitigationStrategies.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('MitigationStrategies', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('MitigationStrategies');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('MitigationStrategies','fetch');
            setStorageItem('MitigationStrategies', 'success', '100','fetch');
        } else { 
            setStorageItem('MitigationStrategies', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchMitigationStrategies
