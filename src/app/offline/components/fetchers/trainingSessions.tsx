import { DownloadTrainingSessions } from '@/app/lib/graphQL/query/offline/downloadTrainingSessions'
...
const FetchTrainingSessions: React.FC<{ flag: string }> = React.memo(
    ({ flag }) => {
        const limit = 100
        ...
        const [queryData] = useLazyQuery(DownloadTrainingSessions, {
            fetchPolicy: 'network-only',
            onCompleted: (response: any) => {
                const data = response.readTrainingSessions.nodes
                const hasNextPage =
                    response.readTrainingSessions.pageInfo.hasNextPage
                const totalCount =
                    response.readTrainingSessions.pageInfo.totalCount
                if (data) {
            ...
            onError: (error: any) => {
                ...
                if (typeof window !== 'undefined') {
                    ...
                    setFetchError('TrainingSessions')
                }
            },
        })
        ...
        useEffect(() => {
            if (progressBarPercentValue == 100) {
                addSuccessResult('TrainingSessions','fetch')
                setStorageItem('TrainingSessions', 'success', '100','fetch')
            } else {
                setStorageItem(
                    'TrainingSessions',
                    'fetching',
