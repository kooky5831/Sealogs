import { DownloadFavoriteLocations } from '@/app/lib/graphQL/query/offline/downloadFavoriteLocations'
...
const FetchFavoriteLocations : React.FC<{ flag: string }> = React.memo(({ flag })  => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'favorite locations'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new FavoriteLocationModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadFavoriteLocations, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readFavoriteLocations.nodes
            const hasNextPage =
                response.readFavoriteLocations.pageInfo.hasNextPage
            const totalCount =
                response.readFavoriteLocations.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('FavoriteLocations', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('FavoriteLocations');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('FavoriteLocations','fetch');
            setStorageItem('FavoriteLocations', 'success', '100','fetch');
        } else {
            setStorageItem('FavoriteLocations', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchFavoriteLocations
