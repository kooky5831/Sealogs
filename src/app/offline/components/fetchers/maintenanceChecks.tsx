import { DownloadMaintenanceChecks } from '@/app/lib/graphQL/query/offline/downloadMaintenanceChecks'
import { useLazyQuery } from '@apollo/client'
import React, { useEffect, useState } from 'react'
import MaintenanceCheckModel from '@/app/offline/models/maintenanceCheck'
import dayjs from 'dayjs'
import { addSuccessResult, setStorageItem, setFetchError } from '../../helpers/functions'
// REMEMBER: The indexedDB is the source of truth
// maintenanceCheck
const FetchMaintenanceChecks : React.FC<{ flag: string }> = React.memo(({ flag }) => {
    const limit = 100
    const [progressBarPercentValue, setProgressBarPercentValue] = useState(0)
    const title = 'maintenance checks'
    const [progressBarLabel, setProgressBarLabel] = useState(
        `Downloading ${title}...`,
    )
    const model = new MaintenanceCheckModel()
    const [page, setPage] = useState(0)
    const [queryData] = useLazyQuery(DownloadMaintenanceChecks, {
        fetchPolicy: 'network-only',
        onCompleted: (response: any) => {
            const data = response.readMaintenanceChecks.nodes
            const hasNextPage =
                response.readMaintenanceChecks.pageInfo.hasNextPage
            const totalCount =
                response.readMaintenanceChecks.pageInfo.totalCount
            if (data) {
        ...
        onError: (error: any) => {
            ...
            if(typeof window !== 'undefined') {
                setStorageItem('MaintenanceChecks', 'error', `${progressBarPercentValue}`,'fetch')
                setFetchError('MaintenanceChecks');
            }
        },
    })
    ...
    useEffect(() => {
        if(progressBarPercentValue == 100) {
            addSuccessResult('MaintenanceChecks','fetch');
            setStorageItem('MaintenanceChecks', 'success', '100','fetch');
        } else { 
            setStorageItem('MaintenanceChecks', 'fetching', `${progressBarPercentValue}`,'fetch');
        }
...
export default FetchMaintenanceChecks
